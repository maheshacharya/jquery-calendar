    ...
    $.ajax({
        ...
        if (data.code === 200) {
            ...
            $(data.data.eventInfo).each(function (i, event) {
                ...
                var eventObject = el.children();

                if(eventObject.length>0){



                    $( $(el).find('.event-details')).append('<div>' + event.startTime + '</div>'
                        + '<div style=\'font-weight:bold\'><a href=\'' + eventlink + '\'>' + event.title + '</a></div>'
                    );

                }else{

                    $(el).append('<div class=\'first-event event-details\' ' +
                        'style=\'min-width:'+($(cal).width()/2)+'px;display:none;padding:10px;position:absolute;background:rgba(250,216,22,.9);z-index:99999;border:1px solid #333\'>'
